...
# Parse arguments
parser = argparse.ArgumentParser(description='Train CNN for sentiment classification')
...
parser.add_argument('--num_filters', type=int, default=100, help='Number of filters (default: 100)')
parser.add_argument('--kernel_sizes', type=int, nargs='+', default=[3, 4, 5],
                    help='Kernel sizes for TextCNN (default: 3 4 5)')
parser.add_argument('--num_conv_layers', type=int, default=3, help='Number of conv layers for TextCNNDeep (default: 3)')
...
if args.output_dir:
    ...
    run_name = f"textcnn_emb{embedding_dim}_f{num_filters}_k{'_'.join(map(str, kernel_sizes))}_{timestamp}"
